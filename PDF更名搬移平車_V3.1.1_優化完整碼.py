# -*- coding: utf-8 -*-
# =========================================================
# PDF更名搬移平車 Ver 3.1.1 精簡修正版 OCR 搬移整合版
# =========================================================
# 保留原功能：
...
class OCREngine:
    ...
    def recognize(self, img: Image.Image) -> str:
        ...
        try:
            arr = np.array(self.preprocess(img))
...
class PDFRenameTool:
    ...
    def create_ui(self):
        ...
        self.center_move_bar.pack(side="left", fill="y", padx=(0, 0), pady=12)
        self.center_move_bar.pack_propagate(False)

        # 右半部：更名預覽 / 搬移瀏覽區堆疊切換
        self.right_shell = tk.Frame(main, bg=BG)
        self.right_shell.pack(side="left", fill="both", expand=True, padx=(6, 6), pady=12)
        ...
        # 最右側縱向分頁標籤：獨立於右半部內容，不會因切換搬移而消失
        self.tab_bar = tk.Frame(main, bg=BG, width=72)
        self.tab_bar.pack(side="left", fill="y", padx=(0, 12), pady=12)
    ...
    def create_treeview(self, parent):
        ...
        self.tree.column("filename", width=430)
    ...
    def create_bottom_rename_area(self, parent):
        ...
        rows = [
            ...
            ("增加前名", self.prefix_var, PREFIX_BG, "增加前名", self.rename_prefix, 1),
        ]
    ...
    def create_move_area(self, parent):
        ...
        self.move_tree.column("name", width=360)
        self.move_tree.column("size", width=110, anchor="e")
        self.move_tree.column("created", width=170, anchor="center")
        self.move_tree.column("modified", width=170, anchor="center")
